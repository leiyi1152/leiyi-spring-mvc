...
@Controller
@RequestMapping(value = "/hotel/level")
public class HotelLevelController extends BaseController {

	@Resource(name = "hotelLevelService")
	private HotelLevelService hotelLevelService;

	String menuUrl = "hotel/level/list.do";
	...
	public ModelAndView toAdd(ModelAndView model) throws Exception {
		...
		if (null!=pd.get("ID")&&!Tools.isEmpty(pd.get("ID").toString())) {
			pd = hotelLevelService.getById(pd);
		}
		model.addObject("pd", pd);
		model.setViewName("hotel/level/edit");
		return model;
	}
	...
	public ModelAndView list(Page page, ModelAndView model,HttpSession session) throws Exception {
		...
		List<PageData> list = hotelLevelService.hotelLevelList(page);
		model.addObject("list", list);
		model.addObject("pd", pd);
		model.setViewName("hotel/level/list");
		return model;
	...
	int hasCode() throws Exception {
		PageData pd = this.getPageData();// 得到页面的请求参数
		int count = hotelLevelService.hasCode(pd);
		return count;
	}
	...
	public String save() throws Exception {
		...
		if (Tools.isEmpty(pd.get("ID").toString())) {
			...
			hotelLevelService.insert(pd);//插入
		} else {
			if(Jurisdiction.buttonJurisdiction(menuUrl, "add"))
			hotelLevelService.update(pd);//更新
		}
		return "save_result";
