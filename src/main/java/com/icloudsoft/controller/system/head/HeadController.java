package com.icloudsoft.controller.system.head;
...
import com.icloudsoft.service.system.appuser.AppuserService;
import com.icloudsoft.service.system.user.UserService;
import com.icloudsoft.util.AppUtil;
import com.icloudsoft.util.Const;
import com.icloudsoft.util.PageData;
import com.icloudsoft.util.SmsUtil;
import com.icloudsoft.util.Tools;
import com.icloudsoft.util.Watermark;
import com.icloudsoft.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public Object sendEmail(){
		...
		if(null != strEMAIL && !"".equals(strEMAIL)){
			String strEM[] = strEMAIL.split(",zw,");
			if(strEM.length == 4){
				if("yes".endsWith(isAll)){
					try {
						List<PageData> userList = new ArrayList<PageData>();
	...
	public ModelAndView goEditEmail() throws Exception{
		...
		if(null != strFWATERM && !"".equals(strFWATERM)){
			String strFW[] = strFWATERM.split(",zw,");
			if(strFW.length == 5){
		...
		if(null != strIWATERM && !"".equals(strIWATERM)){
			String strIW[] = strIWATERM.split(",zw,");
			if(strIW.length == 4){
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",zw,"+pd.getString("PORT")+",zw,"+pd.getString("EMAIL")+",zw,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",zw,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",zw,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",zw,"+pd.getString("fcontent")+",zw,"+pd.getString("fontSize")+",zw,"+pd.getString("fontX")+",zw,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",zw,"+pd.getString("imgUrl")+",zw,"+pd.getString("imgX")+",zw,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
